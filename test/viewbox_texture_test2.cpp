...
void InRender(const Vector2 &offset, const Camera2D &camera) {
	...
	DrawCircle(0  - offset.x, 0 - offset.y, 100 * camera.zoom, ColorAlpha(YELLOW, 1));
	DrawCircle(150 - offset.x, 100 - offset.y, 100 * camera.zoom, ColorAlpha(BLUE, 1));
	DrawEllipse(250 - offset.x, 250 - offset.y, 300 * camera.zoom, 50 * camera.zoom, ColorAlpha(GREEN, 1));
...
int main() {
	...
	while (!WindowShouldClose()) {
		...
		DrawRender(rtex, {drawArea.x, drawArea.y}, camera);

		Rectangle sourceRect = { 0, 0, (float) rtex.texture.width, -(float)rtex.texture.height};
		Rectangle destRect = { sMR.x, sMR.y, sMR.width, sMR.height };

		DrawRectangleLines(sMR.x, sMR.y, sMR.width, sMR.height, BLACK);
		DrawTexturePro(rtex.texture, sourceRect, destRect, {0, 0}, camera.rotation, WHITE);
		EndMode2D();
